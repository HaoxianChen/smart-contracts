...
const testPath = path.join(__dirname, '../tracefiles/controllable/setup.txt');
...
var Controllable = artifacts.require(contractName);
          ...
          if(line != '') {
            ...
            if(eachLineArr[1] == 'constructor') {
              if(fromAccountIndex != -1 && valueTran!= -1) {
                ...
                instance = await Controllable.new(...argArr, {from: accounts[fromAccountIndex], value: valueTran});
              } else if(fromAccountIndex != -1) {
                // console.log('from ');
                instance = await Controllable.new(...argArr, {from: accounts[fromAccountIndex]});
              } else if(valueTran != -1) {
                // console.log('value');
                instance = await Controllable.new(...argArr, {value: valueTran});
              } else {
                // console.log('neither from + value');
                instance = await Controllable.new(...argArr);
              }  
            }
            // call transaction
