...
helper.range(transactionCounts).forEach(l => {
  ...
  if(transactionName == 'swapOwner') {
    ...
    helper.range(tracefileCount).forEach(testFileIndex => {
      ...
      let swapAmount = helper.random(0, mintAmount+1);
      // let text = `swapOwner,constructor,,,${owner},,false\nswapOwner,mint,instance,accounts[${owner}] ${mintAmount},,,false\nswapOwner,swapOwner,instance,accounts[${owner}] accounts[${swapAccountIndex}] ${swapAmount},,,true\n`;
      let text = `swapOwner,constructor,,,${owner},,false\nswapOwner,swapOwner,instance,accounts[${owner}] accounts[${swapAccountIndex}] ${swapAmount},,,true\n`;
      fs.writeFileSync(path.join(transactionFolderPath, fileName), text, function (err) {
        if (err) throw err;
        console.log('File is created successfully.');
