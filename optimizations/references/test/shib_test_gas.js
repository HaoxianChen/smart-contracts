...
const testFolder = path.join(__dirname, `../tracefiles/shib`);
...
helper.range(transactionCounts).forEach(l => {
  ...
  if(transactionName == 'burn') {
    ...
    helper.range(tracefileCount).forEach(testFileIndex => {
      ...
      let text = `burn,constructor,,${name} ${symbol} ${decimal} ${totalSupply} accounts[${feeReceiverIndex}] accounts[${tokenOwnerAddressIndex}],,web3.utils.toWei(${initialTransferAmount} ether),false\nburn,burn,instance,${burnAmount},${tokenOwnerAddressIndex},,true\n`;
  ...
  if(transactionName == 'approve') {
    ...
    helper.range(tracefileCount).forEach(testFileIndex => {
      ...
      let text = `approve,constructor,,${name} ${symbol} ${decimal} ${totalSupply} accounts[${feeReceiverIndex}] accounts[${tokenOwnerAddressIndex}],,web3.utils.toWei(${initialTransferAmount} ether),false\napprove,approve,instance,accounts[${approveAccountIndex}] ${approveAmount},${tokenOwnerAddressIndex},,true\n`;
      fs.writeFileSync(path.join(transactionFolderPath, fileName), text, function (err) {
        if (err) throw err;
        console.log('File is created successfully.');
      });
  ...
  if(transactionName == 'transfer') {
    ...
    helper.range(tracefileCount).forEach(testFileIndex => {
      ...
      let text = `transfer,constructor,,${name} ${symbol} ${decimal} ${totalSupply} accounts[${feeReceiverIndex}] accounts[${tokenOwnerAddressIndex}],,web3.utils.toWei(${initialTransferAmount} ether),false\ntransfer,transfer,instance,accounts[${transferAccountIndex}] ${transferAmount},${tokenOwnerAddressIndex},,true\n`;
      fs.writeFileSync(path.join(transactionFolderPath, fileName), text, function (err) {
        if (err) throw err;
        console.log('File is created successfully.');
      });
  ...
  if(transactionName == 'transferFrom') {
    ...
    helper.range(tracefileCount).forEach(testFileIndex => {
      ...
      let text = `transferFrom,constructor,,${name} ${symbol} ${decimal} ${totalSupply} accounts[${feeReceiverIndex}] accounts[${tokenOwnerAddressIndex}],,web3.utils.toWei(${initialTransferAmount} ether),false\ntransferFrom,approve,instance,accounts[${approveAccountIndex}] ${approveAmount},${tokenOwnerAddressIndex},,false\ntransferFrom,transferFrom,instance,accounts[${tokenOwnerAddressIndex}] accounts[${transferFromToAccountIndex}] ${transferFromAmount},${approveAccountIndex},,true\n`;
      fs.writeFileSync(path.join(transactionFolderPath, fileName), text, function (err) {
        if (err) throw err;
        console.log('File is created successfully.');
      });
